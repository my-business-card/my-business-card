...
function initUploadify( $fileElem, options ) {
    ...
    var defaultButtonText = "Добавить файлы";
...
function getThumbnailUrl( hash, width, height ) {
    ...
    if ( window.fileserviceHost ) {
        result = "//" + window.fileserviceHost + result;
    }
    return result;
}
...
function getDownloadUrl( hash ) {
    // var result = "/fileservice/file/download?hash=" + hash;
    // if ( window.fileserviceHost ) {
    //     result = "//" + window.fileserviceHost + result;
    // }
...
function initListSearch( element, selector ) {
    ...
    var $searchRuleBlock = $("<li style='margin: 0; list-style: none; padding: 5px 3px;'><input class='search-input' placeholder='" + placeholderValue +"' type='text'/><div class='nothing-found-label' style='padding-top: 10px; background: lightyellow; display:none'>ничего не найдено</div></li>")
