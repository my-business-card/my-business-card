$(function() {
	...
	/**
	 * todo что за метод? узнать у тимура
	 * @param obj
	 * @returns {boolean}
	 */
	TreeWidget.moreComments = function(obj) {
		...
		if ( hiddenComments.length ) {
			hiddenComments.slice(hiddenComments.length - 10, hiddenComments.length).removeClass('hide');
		}
		...
		if (length < 1) {
			...
			$(obj).find('span.count').html(length);
		}
	...
	TreeWidget.prototype = {
		...
		,initWebSocketTreeUsers: function ($tree, connection) {
			...
			if (location.hostname == 'kalachevaschool.ru' && '/fashionweek/free' == location.pathname) {
				return;
			}
			...
			connection.subscribeChannel(
				...
				function (message, json) {
					$('.' + treeId).each(function (index, el) {
						...
						var $el = $(el);
						if ($el.hasClass('unique-user-count')) {
							cnt = cnt - 1;
						}
						var html = '';
						if (cnt > 0) {
							html = cnt + ( window.language == 'en' ? ' online' : ' онлайн' );
						}
		...
		,initWebSocketUserTyping : function($tree, connection, oldTreeId) {
			...
			if ($tree.hasClass('comments-tree-show-typing')) {
				...
				connection.subscribeChannel(CHANNELS.TYPING + '_' + $tree.data('tree-id'), function(message, json){
					that.getTypingWidget().addUser(json.user_id, json.user_name, json.user_user_thumbnail);
				});
			}
		},
	...
	CommentFormWidget.prototype = {
		...
		,initUploaders : function(form) {
			...
			this.initUploader(form.find('.comment-attach-file-link'), function($el, $uploader) {
				$uploader.insertAfter($el.parents('.new-comment').find('.btn-send'));
			});
			this.initUploader(form.find('.comment-attach-file-for-edit-link'), function($el, $uploader) {
				$el.append($uploader);
			});
		}
		,initUploader : function($el, insertCallback, afterCreateCallback) {
			var $uploader = $('<div class="file-uploader">');
	...
	CommentWidget.prototype = {
		...
		,initActionsVisual : function($comment) {
			window.eachAndRunOneTimeOnElement(
				$comment.find('.comment-edit-link, .comment-delete-link'),
			...
			window.eachAndRunOneTimeOnElement(
				...
				function ($el) {
					$comment.find('.comment')
						...
						.mouseout(function() {
							$(this).find('.comment-edit-link, .comment-delete-link').addClass('hide');
						})
					;
					return $el;
				}
		...
		var refresh = function() {
			...
			if (messages.length === 0) {
				...
				if ( messages.length > 1 ) {
					...
					$block.html( "<div class='comment-wrapper'> <div class='comment'> <div class='user-image-wrapper'> " + lastTypingUser.user_user_thumbnail + '</div> <div class="text-content"><img src="/public/img/typing.gif"></div>' + "</div></div>");
				}
			}
		};
